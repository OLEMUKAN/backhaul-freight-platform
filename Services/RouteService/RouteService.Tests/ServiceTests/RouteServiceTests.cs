...
namespace RouteService.Tests.ServiceTests
{
    public class RouteServiceTests : IDisposable
    {
        ...
        public async Task CreateRouteAsync_ValidRequest_ReturnsRouteDtoAndPublishesEvent()
        {
            ...
            _mockTruckServiceClient.Setup(ts => ts.VerifyTruckOwnershipAsync(truckId, ownerId)).ReturnsAsync(true);
            _mockTruckServiceClient.Setup(ts => ts.GetTruckCapacityAsync(truckId)).ReturnsAsync((1000m, 50m));
            _mockEventPublisher.Setup(ep => ep.PublishRouteCreatedEventAsync(It.IsAny<Guid>())).Returns(Task.CompletedTask);
            ...
            var result = await _routeService.CreateRouteAsync(request, ownerId);
            ...
            _mockEventPublisher.Verify(ep => ep.PublishRouteCreatedEventAsync(result.Id), Times.Once);
            _mockLogger.Verify(
        ...
        public async Task CreateRouteAsync_InvalidTimes_ThrowsArgumentException()
        {
            ...
            var ex = await Assert.ThrowsAsync<ArgumentException>(() => _routeService.CreateRouteAsync(request, ownerId));
        ...
        public async Task CreateRouteAsync_TruckOwnershipVerificationFails_ThrowsUnauthorizedAccessException()
        {
            ...
            _mockTruckServiceClient.Setup(ts => ts.VerifyTruckOwnershipAsync(truckId, ownerId)).ReturnsAsync(false);
            var ex = await Assert.ThrowsAsync<UnauthorizedAccessException>(() => _routeService.CreateRouteAsync(request, ownerId));
        ...
        public async Task CreateRouteAsync_InvalidOriginPoint_ThrowsArgumentException()
        {
            ...
            _mockGeospatialService.Setup(g => g.ValidatePoint(It.Is<Point>(p => p.X == 200.0))).Returns(false);
             _mockTruckServiceClient.Setup(ts => ts.VerifyTruckOwnershipAsync(request.TruckId, ownerId)).ReturnsAsync(true); // Assume ownership is fine
            _mockTruckServiceClient.Setup(ts => ts.GetTruckCapacityAsync(request.TruckId)).ReturnsAsync((1000m, 50m));
        ...
        {
            var result = await _routeService.GetRouteByIdAsync(Guid.NewGuid());
            Assert.Null(result);
        }
        ...
        public async Task GetRoutesAsync_NoFilter_ReturnsDefaultPaginatedRoutes()
        {
            ...
            var results = (await _routeService.GetRoutesAsync(filter)).ToList();
        ...
        public async Task UpdateRouteAsync_ValidUpdate_ReturnsUpdatedDtoAndPublishesEvents()
        {
            ...
            _mockTruckServiceClient.Setup(ts => ts.VerifyTruckOwnershipAsync(route.TruckId, ownerId)).ReturnsAsync(true);
            _mockEventPublisher.Setup(ep => ep.PublishRouteUpdatedEventAsync(routeId)).Returns(Task.CompletedTask);
            _mockEventPublisher.Setup(ep => ep.PublishRouteStatusUpdatedEventAsync(routeId, RouteStatus.Planned, newStatus)).Returns(Task.CompletedTask);

            var result = await _routeService.UpdateRouteAsync(routeId, updateRequest, ownerId);
            ...
            _mockEventPublisher.Verify(ep => ep.PublishRouteUpdatedEventAsync(routeId), Times.Once);
            _mockEventPublisher.Verify(ep => ep.PublishRouteStatusUpdatedEventAsync(routeId, RouteStatus.Planned, newStatus), Times.Once);
        }
        ...
        public async Task UpdateRouteAsync_UnauthorizedOwner_ThrowsUnauthorizedAccessException()
        {
            ...
            _mockTruckServiceClient.Setup(ts => ts.VerifyTruckOwnershipAsync(route.TruckId, wrongOwnerId)).ReturnsAsync(false);

            await Assert.ThrowsAsync<UnauthorizedAccessException>(() => _routeService.UpdateRouteAsync(routeId, updateRequest, wrongOwnerId));
        }
        ...
        public async Task UpdateRouteCapacityAsync_ValidChange_UpdatesCapacityAndStatusAndPublishesEvents()
        {
            ...
            _mockEventPublisher.Setup(ep => ep.PublishRouteCapacityChangedEventAsync(routeId, 500m, 0m, null, null)).Returns(Task.CompletedTask);
            _mockEventPublisher.Setup(ep => ep.PublishRouteStatusUpdatedEventAsync(routeId, RouteStatus.BookedPartial, RouteStatus.BookedFull)).Returns(Task.CompletedTask);

            var result = await _routeService.UpdateRouteCapacityAsync(routeId, capacityChangeRequest);

            Assert.NotNull(result);
            Assert.Equal(0m, result.CapacityAvailableKg);
            Assert.Equal(RouteStatus.BookedFull, result.Status);

            _mockEventPublisher.Verify(ep => ep.PublishRouteCapacityChangedEventAsync(routeId, 500m, 0m, null, null), Times.Once);
            _mockEventPublisher.Verify(ep => ep.PublishRouteStatusUpdatedEventAsync(routeId, RouteStatus.BookedPartial, RouteStatus.BookedFull), Times.Once);
        }
        ...
        public async Task CancelRouteAsync_ValidRequest_CancelsRouteAndPublishesEvent()
        {
            ...
            _mockTruckServiceClient.Setup(ts => ts.VerifyTruckOwnershipAsync(route.TruckId, ownerId)).ReturnsAsync(true);
            _mockEventPublisher.Setup(ep => ep.PublishRouteStatusUpdatedEventAsync(routeId, RouteStatus.Planned, RouteStatus.Cancelled)).Returns(Task.CompletedTask);

            var success = await _routeService.CancelRouteAsync(routeId, ownerId);

            Assert.True(success);
            var cancelledRoute = await _dbContext.Routes.FindAsync(routeId);
            Assert.Equal(RouteStatus.Cancelled, cancelledRoute.Status);
            _mockEventPublisher.Verify(ep => ep.PublishRouteStatusUpdatedEventAsync(routeId, RouteStatus.Planned, RouteStatus.Cancelled), Times.Once);
        }
        ...
        public async Task CancelRouteAsync_AlreadyCancelled_ReturnsTrue()
        {
            ...
            Assert.True(success);
            _mockEventPublisher.Verify(ep => ep.PublishRouteStatusUpdatedEventAsync(It.IsAny<Guid>(), It.IsAny<RouteStatus>(), It.IsAny<RouteStatus>()), Times.Never);
        }
    }
}
